...
def fix_primary_rai_streams(lines):
    """
    Rai 1 / Rai 2 / Rai 3 usano sempre lo stream corrente delle rispettive
    varianti Europa presenti nella playlist Altervista.

    La riga #EXTINF del canale principale resta invariata; vengono copiati
    soltanto eventuali opzioni e URL dello stream Europa corrispondente.

    Se una variante Europa non esiste nella sorgente, il canale principale
    ...
    # Prima passata: trova opzioni + URL di Rai 1/2/3 Europa.
    while i < len(lines):
        ...
        while j < len(lines) and not lines[j].startswith("#EXTINF"):
            if not lines[j].lstrip("\ufeff").startswith("#EXTM3U"):
                payload.append(lines[j])
            j += 1

        if name in RAI_EUROPA_SOURCE.values():
            europa_payloads[name] = payload

        i = j

    # Seconda passata: sostituisce solo il payload di Rai 1/2/3.
    out = []
    i = 0
    while i < len(lines):
        line = lines[i]
        ...
        source_name = RAI_EUROPA_SOURCE.get(name)

        if source_name and source_name in europa_payloads:
            out.append(line)
            ...
            out.extend(europa_payloads[source_name])
            continue
