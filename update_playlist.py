...
# EPGShare resta la sorgente utilizzata dallo script
# per verificare e correggere i tvg-id.
EPG_SOURCE_URL = "https://epgshare01.online/epgshare01/epg_ripper_IT1.xml.gz"

# Questo è invece l'EPG che Fermata leggerà dalla playlist.
EPG_OUTPUT_URL = "https://raw.githubusercontent.com/dadocadavero-debug/epg-altervista/refs/heads/main/epg.xml"
...
def logo_key(name: str) -> str:
    ...
    toks = [
        t for t in n.split()
        if t not in TECH_WORDS and not re.fullmatch(r"\d+p", t)
    ]
    return " ".join(toks)
    ...
    while i < len(lines):
        ...
        if line.startswith("#EXTINF") and 'group-title="Rai"' in line:
            ...
            if name in RAI_WORKING_STREAMS:
                ...
                # Salta le righe appartenenti al vecchio stream
                # fino al prossimo canale.
                # NON conserva eventuali #EXTM3U.
...
def main():
    # Scarica sempre la playlist Altervista aggiornata.
    m3u = fetch(M3U_URL).decode("utf-8", errors="replace")

    # Scarica EPGShare per verificare i tvg-id.
    ...
    # Impara i loghi già presenti nella playlist Altervista
    # per riusarli automaticamente sui duplicati/backup.
    learned_logos = {}
    ...
    # HEADER
    # ---------------------------------------------------------
    # Mantiene le eventuali altre opzioni dell'header Altervista,
    # ma sostituisce x-tvg-url con il nostro epg.xml.
    #
    # Nell'output deve esserci UNA SOLA riga #EXTM3U.
    # ---------------------------------------------------------
    ...
    if lines and lines[0].startswith("#EXTM3U"):
        ...
        header += f' x-tvg-url="{EPG_OUTPUT_URL}"'
        ...
        out.append(
            f'#EXTM3U x-tvg-url="{EPG_OUTPUT_URL}"'
        )
        start = 0
    ...
    for line in lines[start:]:
        ...
        # Sicurezza:
        # elimina qualsiasi altra intestazione #EXTM3U
        # eventualmente presente nella sorgente.
        ...
        # 2) Mappa ID verificata.
            ...
            old_id
            and old_id in ID_MAP
            and ID_MAP[old_id] in epg_ids
        ):
            new_id = ID_MAP[old_id]
            reason = f"id:{old_id}"

        # 3) Mappa nome verificata.
        elif (
            norm(name) in NAME_MAP
            and NAME_MAP[norm(name)] in epg_ids
        ):
            new_id = NAME_MAP[norm(name)]
            reason = "nome-verificato"

        # 4) Match automatico SOLO se univoco.
        else:
            candidates = names_to_ids.get(
                norm(name),
                set()
            )
        ...
        # Tutti i feed Supertennis+ usano
        # il logo ufficiale di SuperTennis.
        if norm(name).startswith("supertennis+"):
            explicit_logo = LOGO_MAP["supertennis"]

        mapped_logo = best_logo_for_name(
            name,
            learned_logos
        )
        ...
        if wanted_logo and (
            ...
            report.append(
                f"{name} | logo "
                f"{'sostituito' if bad_logo else 'aggiunto'} "
                f"-> {wanted_logo}"
            )
    ...
    while i < len(lines):
        ...
        if line.startswith("#EXTINF") and 'group-title="Rai"' in line:
            ...
            if name in RAI_WORKING_STREAMS:
                ...
                # Elimina URL/opzioni appartenenti al vecchio stream
                # fino al prossimo canale.
                while i < len(lines) and not lines[i].startswith("#EXTINF"):
                    i += 1
