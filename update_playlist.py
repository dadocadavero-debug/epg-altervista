...
# - Rai 1 4K e le altre varianti restano indipendenti;
# - nessun auto-repair/probe sostituisce stream alla cieca;
# - EPG unico: epg.xml del repository;
# - mapping EPG esteso + report dei canali senza guida;
# - ogni canale deve avere un tvg-logo;
# - i mapping logo manuali hanno priorità sui logo sorgente errati.
# ============================================================
...
# Rai 1/2/3 principali prendono dinamicamente i rispettivi feed Europa
# dalla playlist Altervista ad OGNI aggiornamento.
...
def apply_rai_overrides(lines):
    """
    Regola cumulativa definitiva per Rai 1/2/3:
      - Rai 1 usa sempre il payload corrente di "Rai 1 Europa";
      - Rai 2 usa sempre il payload corrente di "Rai 2 Europa";
      - Rai 3 usa sempre il payload corrente di "Rai 3 Europa";
      - le varianti Europa restano nella playlist;
      - Rai 1 4K, Rai 3 900 DASH e tutte le altre varianti NON vengono toccate;
      - nessun URL Rai 1/2/3 è hardcoded.

    Se manca uno dei tre feed Europa o non contiene uno stream, l'aggiornamento
    viene annullato: è preferibile mantenere la playlist precedente funzionante
    piuttosto che produrre una regressione.
    """
    ...
    for main_name, europa_name in RAI_EUROPA_SOURCE.items():
        src = (by_name.get(europa_name) or [None])[0]
        if src is None:
            raise RuntimeError(
                f'Sorgente "{europa_name}" non trovata nella playlist Altervista; '
        ...
        if not payload:
            raise RuntimeError(
                f'Sorgente "{europa_name}" senza stream; aggiornamento annullato.'
            )
        europa_payloads[main_name] = payload

    out_blocks = []
    for block in blocks:
        name = norm(_block_name(block))

        if _is_group_rai(block) and name in europa_payloads:
            out_blocks.append([block[0], *europa_payloads[name]])
        else:
...
def main():
    ...
    # =========================================================
    # 7B. VALIDAZIONI ANTI-REGRESSIONE
    # =========================================================
    # 1) Ogni canale deve avere un logo non vuoto.
    missing_logos = []
    for extinf in output_extinf:
        if not get_tvg_logo(extinf):
            missing_logos.append(channel_name_from_extinf(extinf) or "(senza nome)")

    if missing_logos:
        raise RuntimeError(
            "Validazione loghi fallita. Canali senza tvg-logo: "
            + ", ".join(missing_logos[:20])
            + (f" (+{len(missing_logos) - 20} altri)" if len(missing_logos) > 20 else "")
            + ". Aggiornamento annullato."
        )

    # 2) Primavera TV, se presente, deve usare il logo manuale corretto.
    primavera_expected = LOGO_MAP["primavera tv"]
    for extinf in output_extinf:
        if norm(channel_name_from_extinf(extinf)) == "primavera tv":
            if get_tvg_logo(extinf) != primavera_expected:
                raise RuntimeError(
                    "Regressione Primavera TV: logo diverso dal mapping manuale. "
                    "Aggiornamento annullato."
                )

    # 3) SuperTennis e SuperTennis+ devono usare il logo manuale ufficiale.
    supertennis_expected = LOGO_MAP["supertennis"]
    for extinf in output_extinf:
        cname = norm(channel_name_from_extinf(extinf))
        if (
            cname == "super tennis"
            or cname == "supertennis"
            or cname.startswith("supertennis plus")
        ):
            if get_tvg_logo(extinf) != supertennis_expected:
                raise RuntimeError(
                    f"Regressione logo {channel_name_from_extinf(extinf)}: "
    ...
    if not out or expected_header not in out[0] or expected_header_alt not in out[0]:
        raise RuntimeError(
            "Regressione EPG: header x-tvg-url/url-tvg non corretto. "
            "Aggiornamento annullato."
        )
    ...
    OUT_REPORT.write_text(
        f"Versione logica: MASTER CUMULATIVO 2026-09-26\n"
        f"Rai principali: Rai 1/2/3 <- rispettivi feed Europa Altervista\n"
